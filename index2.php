<?php
/**
 * Anew Era Health — homepage, vibrant variant.
 *
 * Same components, same content, same helpers as index.php — only the colour
 * treatment differs. Sections carry brand colour instead of sitting on white,
 * and the cards cycle through the three brand hues rather than repeating one.
 * Copy lives in includes/data.php, so both pages stay in step.
 * Header and footer come from includes/; all copy comes from includes/data.php.
 */
require __DIR__ . '/includes/header.php';

// Blue, orange, green, in that order. Cards index into this so a row of them
// reads as the brand rather than as one colour repeated.
$accents = [
    ['tint' => 'bg-sky',    'ink' => 'text-brand-blue',        'edge' => 'border-brand-blue'],
    ['tint' => 'bg-blush',  'ink' => 'text-brand-orange-dark', 'edge' => 'border-brand-orange'],
    ['tint' => 'bg-grove',  'ink' => 'text-brand-green-dark',  'edge' => 'border-brand-green'],
];
    ...
    <div class="grid items-stretch gap-6 lg:grid-cols-2">
      ...
      <div class="grid gap-6 sm:grid-cols-2">
        <?php foreach ($results['stats'] as $n => $stat): ?>
          ...
          <div class="flex min-h-[168px] flex-col justify-between rounded-[24px] border border-white/70 bg-white/65 px-7 py-8 shadow-[0_8px_28px_rgba(9,20,28,0.06)] backdrop-blur-sm sm:min-h-[220px]">
            <p class="display-mark m-0 text-[52px] leading-none tracking-[-0.02em] <?= $a['ink'] ?> sm:text-[58px]"><?= e($stat['value']) ?></p>
            <p class="m-0 mt-6 text-[15px] leading-relaxed text-ink/70"><?= e($stat['label']) ?></p>
          </div>
        <?php endforeach; ?>
      </div>
    </div>
    ...
    <div class="flex flex-col gap-2.5">
      <?php foreach ($data['conditions'] as $i => $condition): ?>
        <button type="button"
                ...
                class="grid w-full flex-1 cursor-pointer grid-cols-[minmax(0,1fr)_auto] items-center gap-4 rounded-2xl border border-ink/10 bg-surface px-6 py-5 text-left font-sans text-ink transition-colors hover:border-brand-blue/30 hover:bg-sky data-[active=true]:border-transparent data-[active=true]:bg-brand-blue data-[active=true]:text-white">
...
<!-- ─── Treatments ───────────────────────────────────────────────────── -->
<section id="treatments" class="bg-cream bg-aurora-warm px-5 py-14 sm:px-10 lg:py-[72px]">
  <div class="mx-auto max-w-[1280px]">
    ...
    <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
      <?php foreach ($data['treatments'] as $i => $treatment): ?>
        ...
        <div class="flex flex-col rounded-[22px] border border-white/70 bg-white/70 px-8 pb-8 pt-9 shadow-[0_8px_28px_rgba(9,20,28,0.06)] backdrop-blur-sm">
